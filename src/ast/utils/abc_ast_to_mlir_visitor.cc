...
#include "transpiration/ast/utils/abc_ast_to_mlir_visitor.h"
...
mlir::Type SpecialAbcAstToMlirVisitor::translate_type(Datatype abc_type)
{
    ...
    if (abc_type == Datatype(Type::BOOL))
        ...
        throw runtime_error("Unknown ABC type");
}
...
void SpecialAbcAstToMlirVisitor::recursive_visit(AbstractNode &node, mlir::Block *childBlock)
{
    ...
    {
        throw runtime_error("Unknown subclass of AbstractNode.");
    }
    block = parentBlock;
}
...
void SpecialAbcAstToMlirVisitor::visit(AbstractExpression &expr)
{
    ...
    {
        throw runtime_error("Unknown AbstractExpression type occured in ABC to MLIR translation.");
    }
}
...
void SpecialAbcAstToMlirVisitor::visit(VariableDeclaration &elem)
{
    if (!elem.hasTarget())
    {
        throw runtime_error("Variable declaration must have a target.");
    }
